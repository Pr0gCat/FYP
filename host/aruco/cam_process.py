...
cb_3D_points = np.zeros((cb_width * cb_hight, 3), np.float32)
cb_3D_points[:,:2] = np.mgrid[0:cb_width,0:cb_hight].T.reshape(-1,2) * cb_square_size
...
for frame_name in list_images:
    ...
    if ret == True:
        ...
        cv2.drawChessboardCorners(img,(cb_width, cb_hight), corners2,ret)
        cv2.imshow("img",img)
        cv2.waitKey(500)
